...
/******/ 	// eslint-disable-next-line no-unused-vars
/******/ 	var hotCurrentHash = "4982519f39aa5c687d1f";
...
var sliderButtonsContainer;
var leftBtn;
var rightBtn;
var backDropModal = document.querySelector('.backdrop');
var modalContainer = document.querySelector('.modal');
var soundContainer = document.getElementById('sound-container');
var btnStart = document.createElement('button');
var instructionSliderIndex = 0;
var escContainer = document.getElementById('esc-key-container'); // Slider for changing the content of instructions.

var createSlider = function createSlider(mainModal) {
...
var createMainModal = function createMainModal() {
  ...
  var instructionTitle = document.createElement('h2');
  ...
  var instructionContent = document.createElement('p');
  ...
  var spaceBarContainer = document.createElement('div');
  var titleSpaceBarContainer = document.createElement('h3');
  var spaceBarImg = document.createElement('img');
  ...
  var arrowKeysContainer = document.createElement('div');
  var titleArrowKeyContainer = document.createElement('h3');
  var arrowKeyImg = document.createElement('img');
...
var init = function init() {
  ...
  var rightTimer = null;
  var collectedGifts = [];
  var enemyBulletMovementTimer = null;
  var enemyBulletTimer = null;
  var enemyInterval = null;
  var spaceBarTimer = null;
  ...
  var healthPer = document.getElementById('health');
  var giftsContainer = document.querySelector('#gifts-container');
  healthPer.style.cssText = "background-color:#00FF00;width:100%;";
  var gameBoard = document.getElementById('game');
  gameBoard.appendChild(spaceShip);
  var woundPer = document.getElementById('wound');
  var scoreDisplay = document.getElementById('score');
  var player = new SpaceShip(650, 680, 100, spaceShip, gameBoard, 180);
  var healthContainer = document.createElement('div');
  healthContainer.className = "gift-stats";
  healthContainer.id = "health-gifts";
  var shieldContainer = document.createElement('div');
  shieldContainer.className = "gift-stats";
  shieldContainer.id = "shield-gifts";
  var tenPlusContainer = document.createElement('div');
  tenPlusContainer.className = "gift-stats";
  tenPlusContainer.id = "ten-plus-gifts";
  var superBombContainer = document.createElement('div');
  superBombContainer.className = "gift-stats";
  superBombContainer.id = "super-bomb-gifts";
  var bombNumbersDisplay = document.getElementById('bomb-numbers');
  healthPer.style.cssText = "background-color:#00FF00;width:" + player.health + "%;";
  var playerMissileSound = new Audio('/assets/sounds/player-missile.mp3');
  var enemyMissileExplosionSound = new Audio('/assets/sounds/enemy-missile-explosion.mp3');
  var playerMissileExplosionSound = new Audio('/assets/sounds/player-missile-explosion.mp3');
  var collectItemSound = new Audio('/assets/sounds/collect-item.mp3');
  var isPaused = false;
  var enemies = [];
  var enemyWidth = 64;
  var enemyHeight = 64;
  ...
  var score = 0;
  var enemiesNumbers = 45;
  var injuredNumbers = 0;
  var woundPercent = 0;
  var soundContent = null;
  var soundImgIcon = null;
  var isSoundOff = false;
  var modalBtn = null;
  var isShield = false;
  var isSuperBomb = false;

  var createSoundContainer = function createSoundContainer() {
  ...
  var createEnemies = function createEnemies() {
    var giftsName = ["health", "shield", "ten-plus", "super-bomb"];
    var xPos = 140;
    var yPos = 50;
    var health = 100;
    var enemyContainer;
    var middleEnemyContainer;
    ...
    for (var j = 0; j < 3; j++) {
      ...
      for (var i = 0; i < 15; i++) {
        var myEnemy = void 0;
        var randomSuperAlien = Math.floor(Math.random() * 10);
        ...
        if (randomSuperAlien > 2) {
          ...
          var randomGift = Math.floor(Math.random() * giftsName.length);
  ...
  var enemiesMovement = function enemiesMovement() {
    var randomMovement = Math.floor(Math.random() * 3);
  ...
  var createEnemyFire = function createEnemyFire() {
    var aliveEnemies;
    aliveEnemies = enemies.filter(function (en) {
      return en.isAlive === "alive";
    });

    if (aliveEnemies.length > 0) {
      if (!isPaused) {
        var selectedEnemyIndex = Math.floor(Math.random() * aliveEnemies.length);
        ...
        enemyBulletMovementTimer = setInterval(function () {
          if (!isPaused) {
            aliveEnemies[selectedEnemyIndex].enemy.fireBullet();
            aliveEnemies.map(function (en) {
              return en.enemy.bulletPos.map(function (b, index) {
                if (player.xPos <= b.x && b.x < player.xPos + player.playerWidth && b.y >= player.yPos && b.y < player.yPos + player.playerHeight && isShield === false) {
                  if (!isSoundOff) {
                  ...
                  if (player.health > 0) {
                    ...
                    var modalLooser = new Modal('Game Over', 'Looser', '#9D1627', modalContainer, score, player.health, player.missileNumbers, 'Play Again!', 'looser-card-btn', 'looser-card');
  ...
  enemyBulletTimer = setInterval(function () {
    if (!isPaused) {
      ...
      if (!isSoundOff) {//playerMissileSound.play()
      }
    }
  }, 2000);
  ...
  }, 900); // When Spaceship hits the alien 

  var checkBulletSucceed = function checkBulletSucceed() {
    var spaceShipsBullets;

    if (!isPaused) {
      spaceShipsBullets = player.fireBullet();
      spaceShipsBullets.map(function (bullet, bulletIndex) {
        enemies.map(function (en, index) {
          if (en.enemy.xPos - 4 <= bullet.x && bullet.x <= en.enemy.xPos + enemyWidth && bullet.y + 10 <= en.enemy.yPos + enemyHeight && bullet.y >= en.enemy.yPos && en.isAlive === "alive") {
            if (!isSoundOff) {
  ...
  var winner = function winner() {
    if (enemies.every(function (en) {
      return en.isAlive !== "alive";
    }) && player.health > 0) {
      modalContainer.innerHTML = "";
      var modalWinner = new Modal('Congratulations', 'Winner', '#3FB379', modalContainer, score, player.health, player.missileNumbers, 'Try Again!', "winner-card-btn", "winner-card");
  ...
  var gameOver = function gameOver() {
    modalContainer.innerHTML = "";
    player.health = 0;
    var modalLooser = new Modal('Game Over', 'Looser', '#9D1627', modalContainer, score, player.health, player.missileNumbers, 'Try Again!', 'looser-card-btn', 'looser-card');
    modalBtn = modalLooser.createModal();
    modalContainer.style.display = "block";
    backDropModal.style.display = "block";
    healthPer.style.cssText = "background-color:#00FF00;width:100%;";
  ...
  var checkHitPossibilities = function checkHitPossibilities(timer) {
    if (keysPressed[32]) {
  ...
  var getFrequencyGifts = function getFrequencyGifts(arr, value) {
    return arr.filter(function (v) {
      return v === value;
    }).length;
  };

  var getGifts = function getGifts() {
    if (giftsArr.length > 0) {
      giftsArr.map(function (en, index) {
        if (player.yPos <= en.yPos + 32 && en.xPos >= player.xPos && player.xPos + player.playerWidth >= en.xPos + 32) {
          if (en.className === "health") {
            ...
            var totalShield = getFrequencyGifts(collectedGifts, 'shield');
            var shieldDiv = document.createElement('div');
            var shieldImgIcon = document.createElement('img');
            var shieldTimerDiv = document.createElement('span');
            shieldImgIcon.className = "icon";
            shieldImgIcon.setAttribute('src', '../assets/images/shield.png');
            shieldDiv.appendChild(shieldImgIcon);
            var shieldStats = document.createElement('span');
            ...
            var shieldTimerId = setInterval(function () {
              shieldTime -= 1;
            ...
            var totalTenPlus = getFrequencyGifts(collectedGifts, 'ten-plus');
            var tenPlusDiv = document.createElement('div');
            var tenPlusImgIcon = document.createElement('img');
            ...
            var tenPlusStats = document.createElement('span');
            ...
            var totalSuperBomb = getFrequencyGifts(collectedGifts, 'super-bomb');
            var superBombDiv = document.createElement('div');
            var superBombImgIcon = document.createElement('img');
            var superBombTimeDisplay = document.createElement('span');
            ...
            var superbombStats = document.createElement('span');
            ...
            var superBombTimerId = setInterval(function () {
              superBombTime -= 1;
  ...
  var giftsTimer = setInterval(function () {
    getGifts();
  }, 200);

  var isSpaceShipTouchedAliens = function isSpaceShipTouchedAliens() {
    enemies.map(function (en) {
      if (player.yPos <= en.enemy.yPos + enemyHeight && en.enemy.xPos <= player.xPos && player.xPos <= en.enemy.xPos + enemyWidth && isShield === false) {
        if (en.isAlive === "alive") {
          playerMissileExplosionSound.play();
          playerMissileExplosionSound.play();
          player.health = 0;
  ...
  document.body.addEventListener('keydown', function (e) {
    ...
    if (e.keyCode in keysPressed) {
      keysPressed[e.keyCode] = true; //When user press the left arrow key and up arrow key 
                  ...
                  if (!isPaused) {
                    ...
                    if (!isPaused) {
                      ...
                      if (!isPaused) {
                        ...
                        modalContainer.innerHTML = "";
                        var modalPause = new Modal('Space Invaders', 'Killer', '#006a71', modalContainer, score, player.health, player.missileNumbers, 'Continue!', 'looser-card-btn', 'looser-card', backDropModal);
  ...
  soundImgIcon.addEventListener('click', function () {
